...
await client.execute('create table if not exists tuplebase.crew (id int primary key, name text, role text)')
for (const [id, name, role] of [[1, 'ada', 'captain'], [2, 'grace', 'navigator'], [3, 'hedy', 'engineer']]) {
  await client.execute('insert into tuplebase.crew (id, name, role) values (?, ?, ?)', [id, name, role], { prepare: true })
}
await client.shutdown()
console.log('seeded cassandra: tuplebase.crew (3 rows)')
